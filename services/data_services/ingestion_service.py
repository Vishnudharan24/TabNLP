...
def run_ingestion(source_id=None, url=None):
    ...
    raw_data, headers = fetch_data(url)

    df = parse_data(raw_data, headers.get("content-type", ""))

    metadata = generate_metadata(url, df)
    if source_id:
        metadata["source_id"] = source_id

    storage_result = store_dataset(metadata, df, source_id=source_id)
